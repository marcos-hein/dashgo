...
function generatePagesArray(from: number, to: number) {
  return [...new Array(to - from)].map((_, index) => from + index + 1).filter(page => page > 1);
}
    ...
    <Stack direction={["column", "row"]} mt="8" justify="space-between" align="center" spacing="6">
      ...
      <HStack spacing="2">
        ...
        {previousPages.length > 0 &&
          previousPages.map(page => <PaginationItem key={page} number={page} />)}

        <PaginationItem number={currentPage} isCurrent />

        {nextPages.length > 0 && nextPages.map(page => <PaginationItem key={page} number={page} />)}
